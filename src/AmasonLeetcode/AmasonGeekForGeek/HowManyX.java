 ...
 * excluding the bounds. Print the frequency of occurrence as output.

 Input:
 The first line of input is an integer T, denoting the number of test cases.
 For each test case, there are two lines of input, first consisting of the integer X,
 whose occurrence has to be counted. Second, the lower and upper bound,
 L and U which are positive integers,
 on the same line separated by a single space, respectively.
 ...
 the occurrence of 3 in the numbers starting from 101 to 249 is counted and comes out to be 35.
...
public class HowManyX {
    public static void main(String[] args){
        ...
        while(t>0){
            t--;
            char X = (char)(sc.nextInt()+'0');
            int l = sc.nextInt();
            int r = sc.nextInt();
            int count = 0;
            l++; //don't need the boundary
            while(l<r){
                char[] dig = (l+"").toCharArray();
                for(char ch:dig){
                    if(ch == X)
                        count++;
                }
